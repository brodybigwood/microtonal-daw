...
void VstPlugin::serialize(std::filesystem::path folder) {

    std::vector<uint8_t> vstState = getState();

    std::ofstream stateFile(folder / "stateFile", std::ios::binary);
    if (stateFile.is_open()) {
        stateFile.write(reinterpret_cast<const char*>(vstState.data()), vstState.size());
    }
}

void VstPlugin::deSerialize(std::filesystem::path folder) {
    

    std::ifstream in(folder / "stateFile", std::ios::binary | std::ios::ate);
    if (!in.is_open()) {
...
void VstPlugin::setState(std::vector<uint8_t>& stateData) {
...
std::vector<uint8_t> VstPlugin::getState() {
    VectorStream stream;
    if (component) {
...
void VstPlugin::process(audioData audio, EventList* eventList) {
...
bool VstPlugin::instantiatePlugin() {
...
bool VstPlugin::createEditControllerAndPlugView(const Steinberg::TUID controllerCID) {
